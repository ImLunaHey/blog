...
import { Head, HeadTag, TestContext, TestProvider, useTestContext } from './context';
...
const App: React.FC<PropsWithChildren> = ({ children }) => {
  ...
  // We need to render the app twice to get the head elements
  renderToStaticMarkup(<AppInner context={context}>{children}</AppInner>);
  const titleElement = context.elements.find((element) => {
    const elementAsReactElement = renderToString(<>{element}</>);
    return elementAsReactElement.startsWith('<title>');
  });
  const headElements = context.elements.filter((element) => element !== titleElement);
  const title = titleElement ? renderToString(<>{titleElement}</>).replace(/<\/?title>/g, '') : null;
  return (
    <html lang={blog.language}>
      ...
      <AppInner context={context}>{children}</AppInner>;
